...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
            ...
            for (Method currentMethod : clazz.getDeclaredMethods()) {
                if (currentMethod.getName().equals(route.getAction())) {
                    ...
                    for (Class<?> parameterType : currentMethod.getParameterTypes()) {
                        if (parameterType != HttpServletRequest.class && parameterType != Map.class) {
                            found = false;
                            break;
                        }
                    }
        ...
        Template template = getTemplateByLanguage(request, pageClass);
    ...
    private Template getTemplateByLanguage(HttpServletRequest request, Class<?> pageClass)
            throws ServletException {
        String lang = (String) request.getSession().getAttribute("lang");
        Template template = null;
        String defaultTemplateName = String.format("%s.ftlh", pageClass.getSimpleName());
        if (lang != null) {
            template = newTemplate(String.format("%s_%s.ftlh", pageClass.getSimpleName(), lang));
        }
        if (template == null) {
            template = newTemplate(defaultTemplateName);
        }
        if (template == null) {
            throw new ServletException("Can't find template [templateName=" + defaultTemplateName + "]");
        }
        return template;
    }

    private void processLanguage(HttpServletRequest request) {
        String language = request.getParameter("lang");
        if (language == null || language.length() != 2) {
            return;
        }
        for (int i = 0; i < language.length(); i++) {
            if (!validLangChar(language.charAt(i))) {
                return;
            }
        }
    ...
    private boolean validLangChar(char c) {
