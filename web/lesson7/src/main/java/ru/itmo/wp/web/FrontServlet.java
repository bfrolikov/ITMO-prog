...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        methods.put("before", getMethod(pageClass, "before"));
        methods.put("action", getMethod(pageClass, route.getAction()));
        methods.put("after", getMethod(pageClass, "after"));

        if (methods.containsValue(null)) {
            throw new NotFoundException();
        }

        String acceptRequestHeader = request.getHeader("Accept");
        boolean json = methods.get("action").getAnnotation(Json.class) != null
        ...
        if (json) {
            ...
            try {
                ...
                throw new ServletException("Can't render template [pageClass=" + pageClass + ", method=" + methods.get("action") + "].", e);
            }
        }
    }
